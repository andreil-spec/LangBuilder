...
A `Data` object stores key-value pairs within the `.data` attribute, where each key is a field name and its value can be any supported data type. `text_key` tells Langbuilder which key in the data dictionary is the primary text value for that object.
