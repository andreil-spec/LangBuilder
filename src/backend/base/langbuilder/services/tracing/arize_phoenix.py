...
class ArizePhoenixTracer(BaseTracer):
    ...
    def __init__(
        ...
        try:
            ...
            self.root_span.set_attribute("langbuilder.project.name", self.project_name)
            self.root_span.set_attribute("langbuilder.flow.name", self.flow_name)
            self.root_span.set_attribute("langbuilder.flow.id", self.flow_id)
