...
def main():
    """Launches langbuilder with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from langbuilder.__main__ import main as langbuilder_main

        langbuilder_main()


def _launch_with_exec():
    """Launch langbuilder by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "langbuilder.__main__"] + sys.argv[1:])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec langbuilder: {e}", file=sys.stderr)
        sys.exit(1)
