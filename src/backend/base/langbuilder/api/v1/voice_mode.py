...
from langbuilder.api.v1.chat import build_flow_and_stream
...
from langbuilder.services.auth.utils import get_current_user_for_websocket
...
from langbuilder.services.deps import get_variable_service, session_scope
from langbuilder.utils.voice_utils import (
    BYTES_PER_24K_FRAME,
    VAD_SAMPLE_RATE_16K,
    resample_24k_to_16k,
)
...
async def authenticate_and_get_openai_key(session: DbSession, user: User, websocket: WebSocket):
    ...
    if user is None:
        await websocket.send_json(
            {
                "type": "error",
                "code": "langbuilder_auth",
                "message": "You must pass a valid Langbuilder token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "langbuilder.voice_mode.config":
                            logger.info(f"langbuilder.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "langbuilder.elevenlabs.config":
                            logger.info(f"langbuilder.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "langbuilder.elevenlabs.config":
                            logger.info(f"langbuilder.elevenlabs.config {event}")
