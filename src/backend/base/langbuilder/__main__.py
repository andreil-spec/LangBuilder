...
from langbuilder.cli.progress import create_langbuilder_progress
from langbuilder.initial_setup.setup import get_or_create_default_folder
from langbuilder.logging.logger import configure, logger
from langbuilder.main import setup_app
...
from langbuilder.utils.version import fetch_latest_version, get_version_info
...
def run(
    *,
    ...
    env_log_level = os.environ.get("LANGBUILDER_LOG_LEVEL")
    if env_log_level is None:
        os.environ["LANGBUILDER_LOG_LEVEL"] = log_level_str
    else:
        os.environ["LANGBUILDER_LOG_LEVEL"] = env_log_level.lower()
    ...
    progress = create_langbuilder_progress(verbose=verbose)

    # Step 0: Initializing Langbuilder
    ...
    with progress.step(1):
        for key, value in os.environ.items():
            new_key = key.replace("LANGBUILDER_", "")
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            # Use Gunicorn with LangbuilderUvicornWorker for non-Windows systems
            from langbuilder.server import LangbuilderApplication
            ...
            server = LangbuilderApplication(app, options)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "langbuilder")
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use langbuilder.utils.version to get the version info
    version_info = get_version_info()
    langbuilder_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= langbuilder_is_pre_release(langbuilder_version)  # Update pre-release status

    notice = build_version_notice(langbuilder_version, package_name)
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("LANGBUILDER_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Langbuilder.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open Langbuilder {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="LANGBUILDER_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
...
# command to copy the langbuilder database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'langbuilder.db' and 'langbuilder-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "langbuilder.db"
    pre_db_path = cache_dir / "langbuilder-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from langbuilder.services.database.models.api_key.crud import create_api_key, delete_api_key
            from langbuilder.services.database.models.api_key.model import ApiKey, ApiKeyCreate
...
def show_version(*, value: bool):
    if value:
        ...
        typer.echo(f"langbuilder {version}")
        raise typer.Exit
